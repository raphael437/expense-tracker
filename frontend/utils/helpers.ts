...
const fetchTranctionList = (list: ITransactionData[] = []) => {
...
const getDahsboardValues = async (tokin: string) => {
  const incomeList = await fetchIncome(tokin);
  const expenseList = await fetchExpense(tokin);
  let incomeValue = 0;
  let expenseValue = 0;
...
const getMoneyFlowOptions=(categories:string[],incomeSeries:number[],expenseSeries:number[]):Highcharts.Options=>{
  return {
    chart:{
      type:'column',
      backgroundColor:'transparent',
